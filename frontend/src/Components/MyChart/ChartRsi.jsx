...
const ChartRsi = ({ selectedDateStart, selectedDateEnd }) => {
    const { rsi, rsiTime, simulationValueDataRsi, simulationLabelDataRsi, windowSize, rsiComplete } = useContext(AppContext);
    ...
    const values = new Float64Array(rsiComplete.length);
    const label = new Array(rsiComplete.length);

    for (let i = 0; i < rsiComplete.length; i++) {
        values[i] = rsiComplete[i].rsi_ma;
        label[i] = rsiComplete[i].time;
    }
    ...
    const activeValue = simulationValueDataRsi?.length > 0 ? simulationValueDataRsi : values;
    const activeLabel = simulationLabelDataRsi?.length > 0 ? simulationLabelDataRsi : labels;
