...
const CandleChart = () => {
  const { simulationCandleComplete, candlesComplete, windowSize, symbol, startGlobalLoading, stopGlobalLoading, } = React.useContext(AppContext);
  ...
  const [chartKey, setChartKey] = React.useState(`chart-${symbol}`);

  // NOVO: Ref para controlar o último dataVersion que foi renderizado
  const lastRenderedVersionRef = React.useRef(0);
  ...
  const zoomModeRef = React.useRef('xy');
  ...
  const candleData = React.useMemo(() => {
    // Alterna entre simulationCandleComplete e candlesComplete: se um estiver vazio, usa o outro
    const selectedData = simulationCandleComplete?.length > 0
      ? simulationCandleComplete
      : candlesComplete;

    if (!selectedData || selectedData.length === 0) {return [];}

    // Garantir que windowSize é um número positivo
    const size = Math.max(1, Number(windowSize) || 100);
    // Pegar apenas os últimos 'size' elementos
    const visibleValues = selectedData.slice(-size);

    const raw = Array.isArray(visibleValues) ? visibleValues : [visibleValues];

    const parsed = raw.map((item) => {
      // ⚠️ IMPORTANTE: USAR EXATAMENTE OS NOMES QUE ESTÃO SENDO ENVIADOS
      // O formato PADRÃO agora é: open, high, low, close, tempo, volume

      // Tenta pegar nos formatos possíveis, priorizando o formato correto
      const o = item.open ?? item.Open ?? item.o ?? item.Abertura ?? 0;
      const h = item.high ?? item.High ?? item.h ?? item.Maximo ?? 0;
      const l = item.low ?? item.Low ?? item.l ?? item.Minimo ?? 0;
      const c = item.close ?? item.Close ?? item.c ?? item.Fechamento ?? 0;
      const t = item.tempo ?? item.Tempo ?? item.time ?? item.date;
      const v = item.volume ?? item.Volume ?? item.vol ?? 0;

      // Converte timestamp
      const date = t ? new Date(String(t).replace(' ', 'T')) : null;
    ...
    return parsed;
  }, [simulationCandleComplete, candlesComplete, windowSize]);



  // FORÇA ATUALIZAÇÃO COMPLETA DO GRÁFICO
  React.useEffect(() => {
    const chart = chartRef.current;
    if (!chart || !candleData.length) return;

    // Só atualiza se for uma nova versão dos dados
    if (lastRenderedVersionRef.current === dataVersionRef.current) return;

    // MOSTRA OS ÚLTIMOS CANDLES PARA DEBUG
    if (candleData.length > 3) {
      console.log('🕯️ Últimos 3 candles:', candleData.slice(-3));
    }

    // Reset dos limites dos eixos antes de atualizar
    if (chart.options.scales) {
      // Remove limites manuais para recalcular automaticamente
    ...
    lastRenderedVersionRef.current = dataVersionRef.current;

  }, [candleData]);

  // Atualiza a key apenas quando símbolo mudar
  React.useEffect(() => {
    setChartKey(`chart-${symbol}`);
    // Reset da versão quando muda o símbolo
    lastRenderedVersionRef.current = 0;
  }, [symbol]);
  ...
  const toggleZoomMode = () => {
    ...
    const newMode = zoomModeRef.current === 'xy' ? 'y' : 'xy';
    zoomModeRef.current = newMode;

    if (chart.options.plugins && chart.options.plugins.zoom) {
      chart.options.plugins.zoom.pan.mode = newMode;
  ...
  return (
    <div className="chart-candle" ref={containerRef} >
      <div className="draw-toolbar" style={{ position: 'absolute', top: 10, left: 12, zIndex: 20, display: 'flex', gap: 15, alignItems: 'center' }}>
      ...
      <button
        ...
        className={zoomModeRef.current === 'y' ? '' : ''}
        style={{
          backgroundColor: zoomModeRef.current === 'y' ? '#5e5e5e' : 'transparent',
          color: zoomModeRef.current === 'y' ? '#eeb7ff' : '#ffffff',
      ...
      >
        {zoomModeRef.current === 'xy' ? '🔒 Zoom XY' : '📏 Zoom Y (Resize)'}
      </button>
      ...
      <div className='chart-candle'>
        ...
        <div className='chart-container' >
          <Chart
            ...
            options={options}
            plugins={[{
              afterUpdate: (chart) => { console.log('✅ Chart atualizado no DOM', chart); }
            }]}
          />
          <canvas
            ref={overlayRef}
